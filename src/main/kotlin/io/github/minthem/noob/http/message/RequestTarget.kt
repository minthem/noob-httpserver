...
data class RequestTarget internal constructor(
...
internal object RequestTargetParser {
    ...
    private enum class RequestTargetState {
        START {
            override fun next(b: Byte): RequestTargetState = if (b == SLASH) PATH else INVALID
        },
        PATH {
            override fun next(b: Byte): RequestTargetState =
                if (LOOKUP_UNRESERVED[b.toInt() and 0xFF] ||
                    LOOKUP_SUB_DELIMITERS[b.toInt() and 0xFF] ||
                    b == COLON ||
                    b == AT ||
                    b == SLASH
                ) {
        ...
        PATH_PCT1 {
            override fun next(b: Byte): RequestTargetState = if (LOOKUP_HEX_DIGITS[b.toInt() and 0xFF]) PATH_PCT2 else INVALID
        },
        PATH_PCT2 {
            override fun next(b: Byte): RequestTargetState = if (LOOKUP_HEX_DIGITS[b.toInt() and 0xFF]) PATH else INVALID
        },
        QUERY {
            override fun next(b: Byte): RequestTargetState =
                if (LOOKUP_UNRESERVED[b.toInt() and 0xFF] ||
        ...
        QUERY_PCT1 {
            override fun next(b: Byte): RequestTargetState = if (LOOKUP_HEX_DIGITS[b.toInt() and 0xFF]) QUERY_PCT2 else INVALID
        },
        QUERY_PCT2 {
            override fun next(b: Byte): RequestTargetState = if (LOOKUP_HEX_DIGITS[b.toInt() and 0xFF]) QUERY else INVALID
        },
        END {
            override fun next(b: Byte): RequestTargetState = this
        },
        INVALID {
            override fun next(b: Byte): RequestTargetState = this
        }, ;
        ...
        companion object {
            ...
            private val LOOKUP_SUB_DELIMITERS =
                BooleanArray(256) { false }.apply {
                    for (i in "!$&'()*+,;=") this[i.code] = true
                }

            private val LOOKUP_HEX_DIGITS =
                BooleanArray(256) { false }.apply {
                    for (i in '0'..'9') this[i.code] = true
